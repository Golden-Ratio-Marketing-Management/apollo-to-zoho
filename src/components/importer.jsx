...
export default function Importer() {
  ...
  async function fetchAccountInfo(ids = []) {
    ...
    settled.forEach((s, i) => {
      if (s.status === "fulfilled") {
        ...
        console.error(
          `Failed to fetch account info for org ${orgIDs[i]}`,
          s.reason,
        );
      }
    });
  ...
  async function onPull() {
    ...
    setContacts((prev) =>
      prev.map((c) =>
        c.checked ? { ...c, status: "pushing", errors: undefined } : c,
      ),
    );
    ...
    try {
      const accountsInfo = await fetchAccountInfo(
        contactsToPush.map((c) => c.orgID),
      );
        ...
        return contactToZohoItem(
          ...
          org
            ? {
                ...
                orgSizeRange: employeeCountToRange(
      ...
      try {
        ...
        zohoResults =
          normalizeZohoInsertResponse(insertErr) ??
          normalizeZohoInsertResponse(insertErr?.data);

        if (zohoResults === null) {
          console.error(
            "Could not recover results from thrown error:",
            insertErr,
          );
          unrecoverable = true;
        }
          ...
          prev.map((c) =>
            contactsToPush.some((r) => r.id === c.id)
              ? {
                  ...c,
                  status: "error",
                  errors: ["Unrecognized response from Zoho"],
                }
              : c,
          ),
        );
      ...
      const { resolved, successCount, errorCount, duplicateCount } =
        mapZohoPushResults(contactsToPush, zohoResults);
      console.log(
        "resolved map:",
        resolved,
        "success:",
